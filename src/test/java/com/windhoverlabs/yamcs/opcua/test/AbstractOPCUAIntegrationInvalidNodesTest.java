 ...
 * @author lgomez
 */
public abstract class AbstractOPCUAIntegrationInvalidNodesTest {

  protected final String yamcsHost = "localhost";
  protected final int yamcsPort = 9190;
  protected final String yamcsInstance = "IntegrationTestInvalidNodes";
  ...
  protected void setupYamcs() throws Exception {
    Path dataDir =
        Path.of(System.getProperty("java.io.tmpdir"), "yamcs-IntegrationTestInvalidNodes-data");
    FileUtils.deleteRecursivelyIfExists(dataDir);

    YConfiguration.setupTest("IntegrationTestInvalidNodes");
  ...
  void generatePkt13AndPps(String utcStart, int numPackets) {
    ...
    for (int i = 0; i < numPackets; i++) {
      ...
      // parameters are 10ms later than packets to make sure that we have a predictable order during
      // replay
  ...
  public static class ParameterProvider implements ParameterDataLink {
    ...
    void generateParameters(int x) {
      ...
      ParameterValue pv1 =
          new ParameterValue(xtcedb.getParameter("/REFMDB/SUBSYS1/processed_para_uint"));
      pv1.setUnsignedIntegerValue(x);
      pv1.setGenerationTime(generationTime);

      ParameterValue pv2 =
          new ParameterValue(xtcedb.getParameter("/REFMDB/SUBSYS1/processed_para_string"));
      pv2.setGenerationTime(generationTime);
      pv2.setStringValue("para" + x);

      // add a parameter raw value to see that it's calibrated
      ParameterValue pv5 =
          new ParameterValue(xtcedb.getParameter("/REFMDB/SUBSYS1/processed_para_enum_nc"));
      pv5.setGenerationTime(generationTime);
      pv5.setRawUnsignedInteger(1);

      ppListener.updateParameters(
          generationTime, "IntegrationTestInvalidNodes", seqNum, Arrays.asList(pv1, pv2, pv5));

      // this one should be combined with the two above in the archive as they have the same
      // generation time,
      // group and sequence
      org.yamcs.protobuf.Pvalue.ParameterValue pv3 =
          org.yamcs.protobuf.Pvalue.ParameterValue.newBuilder()
      ...
      ppListener.updateParams(
          generationTime, "OPCUALinkInvalidNodesTest", seqNum, Arrays.asList(pv3));
      ...
      org.yamcs.protobuf.Pvalue.ParameterValue pv4 =
          org.yamcs.protobuf.Pvalue.ParameterValue.newBuilder()
              ...
              .build();
      ppListener.updateParams(
          generationTime + 20, "OPCUALinkInvalidNodesTest", seqNum, Arrays.asList(pv4));
